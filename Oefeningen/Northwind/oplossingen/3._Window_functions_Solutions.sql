...
COUNT(orderID) OVER (PARTITION BY CustomerID ORDER BY YEAR(Orderdate)) TotalOrders
FROM Orders
...
COUNT(supplierid) OVER (partition by country order by companyname) 
FROM suppliers
ORDER BY country;
...
SELECT country, NumberOfCustomersPerCountry * 1.0 / TotalNumberOfCustomers
FROM cte_1, cte_2
